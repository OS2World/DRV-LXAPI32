...
#ifdef TARGET_OS2
static void* LX_malloc(unsigned long size)
{
 void* p;
 if(DosAllocMem(&p,size,PAG_COMMIT | PAG_READ | PAG_WRITE))
  return 0;
 return p;
}
static void LX_free(void* ptr)
{
 DosFreeMem(ptr);
}
                                    ...
                                    USHORT usCodepage)  // in: codepage to set in EAT_MVMT
{
    ...
    if (pcszInput)
    {
        if (0!=(peab = (PEABINDING)malloc(sizeof(EABINDING))))
        {
            ...
            while (0!=(p = strstr(p, pcszSeparator)))
            {
                usSepCount++;
                p += cbSep;
            }
