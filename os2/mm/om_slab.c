/* $Id: om_slab.c,v 1.10 2005/03/27 20:59:37 smilcke Exp $ */

/*
 * lm_slab.c
 * Autor:               Stefan Milcke
 * Erstellt am:         21.09.2004
 * Letzte Aenderung am: 21.03.2005
 *
*/
...
#if defined(TARGET_OS2) && !defined(DEBUG)
#define cache_free_debugcheck(cachep,objp,caller) (objp)
#else
...
#if defined(TARGET_OS2) && !defined(DEBUG)
#define check_slabp(cachep,slabp)
#else // TARGET_OS2
...
/* There seems to be a problem with low memory condition resulting in an endless
   loop. To prevent this I've introduced prev_entry and some checking
...
#ifdef TARGET_OS2
      if(prev_entry==entry || (entry == &l3->slabs_partial)) {
#else
...
#if defined(TARGET_OS2) && !defined(DEBUG)
#define cache_alloc_debugcheck_after(cachep,flags,objp,caller) (objp)
#else // TARGET_OS2
static inline void *
...
void * __kmalloc (size_t size, int flags)
{
   ...
   for (; csizep->cs_size; csizep++) {
       ...
       * Whe route this directly to LX_malloc_gfp()
       */
   if(csizep->cs_cachep==NULL)
    return LX_malloc_gfp(size,flags);
#endif // TARGET_OS2
#if DEBUG
      /* This happens if someone tries to call
       * kmem_cache_create(), or kmalloc(), before
