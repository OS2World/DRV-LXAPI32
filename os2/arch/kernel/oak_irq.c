/* $Id: oak_irq.c,v 1.5 2005/12/13 22:24:48 smilcke Exp $ */

/*
 * oak_irq.c
 * Autor:               Stefan Milcke
 * Erstellt am:         21.09.2004
 * Letzte Aenderung am: 07.01.2005
 *
*/
      ...
      struct pt_regs *regs, struct irqaction *action)
{
   ...
   do {
      ...
      action = action->next;
#ifdef TARGET_OS2
      if((retval & IRQ_HANDLED) && 0==lx_irq_handled)
      {
       LX_set_irq_handled(irq);
       lx_irq_handled=1;
      }
#endif
   } while (action);
...
{
#ifdef TARGET_OS2
 /* In OS/2 there may be other drivers out of LXAPI32.SYS.
    So never check if an IRQ is unhandled
  */
#else
...
#ifdef TARGET_OS2
   if(p)
    LX_RequestIrq(irq,((*p)->flags & SA_SHIRQ) ? 1 : 0);
#endif
   spin_unlock_irqrestore(&desc->lock,flags);
