/* $Id: ok_sched.c,v 1.8 2005/06/21 23:04:37 smilcke Exp $ */

/*
 * ok_sched.c
 * Autor:               Stefan Milcke
 * Erstellt am:         28.06.2004
 * Letzte Aenderung am: 22.06.2005
 *
*/
 ...
 */
void fastcall LX_sched_fork(task_t* from,task_t* p)
{
 if(0==from)
  from=current;
   /*
    ...
    */
   local_irq_disable();
   p->time_slice = (from->time_slice + 1) >> 1;
   /*
...
switch_tasks:
...
#ifdef TARGET_OS2
      if(prev && ((prev->state==TASK_ZOMBIE) || (prev->state==TASK_DEAD)))
       return;
#endif
      barrier();
   ...
   {
    /* Note:
       In OS/2 there are other threads that could be runned not known
       by LXAPI32 including some service threads in LXAPI32 itself.
       So we MUST block here!
     */
      spin_unlock_irq(&rq->lock);
      LX_set_current_task(current);
      DevBlock((unsigned long)current,10,0);
      LX_call_shrinkers(10);
   }
#else
      spin_unlock_irq(&rq->lock);
#endif
